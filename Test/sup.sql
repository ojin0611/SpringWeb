...
CREATE TABLE sup.member (
	...
	member_pwd VARCHAR2(15) NOT NULL, /* 비밀번호 */
	...
	member_businessid VARCHAR2(30), /* 사업자등록번호 */
...
COMMENT ON COLUMN sup.member.member_pwd IS '비밀번호';
...
COMMENT ON COLUMN sup.member.member_businessid IS '사업자등록번호';
...
CREATE TABLE sup.product (
	...
	product_introduction VARCHAR2(4000) NOT NULL, /* 상품 상세설명 */
...
COMMENT ON COLUMN sup.product.product_introduction IS '상품 상세설명';
...
/* 구매 */
CREATE TABLE sup.order (
	order_id NUMBER(38) NOT NULL, /* 구매 번호 */
	product_id NUMBER(38) NOT NULL, /* 상품 아이디 */
	sender_id VARCHAR2(20) NOT NULL, /* 선물한사람ID */
	recepient_id VARCHAR2(20) NOT NULL, /* 선물받은사람ID */
	order_state CHAR(1) NOT NULL, /* 주문 상태(사용/미사용/거절) */
	order_date DATE NOT NULL /* 구매 날짜 */
);

COMMENT ON TABLE sup.order IS '구매';

COMMENT ON COLUMN sup.order.order_id IS '구매 번호';

COMMENT ON COLUMN sup.order.product_id IS '상품 아이디';

COMMENT ON COLUMN sup.order.sender_id IS '선물한사람ID';

COMMENT ON COLUMN sup.order.recepient_id IS '선물받은사람ID';

COMMENT ON COLUMN sup.order.order_state IS '주문 상태(사용/미사용/거절)';

COMMENT ON COLUMN sup.order.order_date IS '구매 날짜';

CREATE UNIQUE INDEX sup.PK_order
	ON sup.order (
		order_id ASC
	);

ALTER TABLE sup.order
	ADD
		CONSTRAINT PK_order
		PRIMARY KEY (
			order_id
...
CREATE TABLE sup.post (
	...
	post_shared_range CHAR(1), /* 공유 범위 */
	post_like_count NUMBER(38), /* 글 좋아요 수 */
...
COMMENT ON TABLE sup.like_feed IS '글 좋아요';

COMMENT ON COLUMN sup.like_feed.like_post_id IS '좋아요 기본키';

COMMENT ON COLUMN sup.like_feed.post_id IS '글 아이디';

COMMENT ON COLUMN sup.like_feed.member_id IS '좋아요한 회원 id';

COMMENT ON COLUMN sup.like_feed.like_post_date IS '좋아요 날짜';

CREATE UNIQUE INDEX sup.PK_like_feed
	ON sup.like_feed (
		like_post_id ASC
	);

ALTER TABLE sup.like_feed
	ADD
		CONSTRAINT PK_like_feed
		PRIMARY KEY (
			like_post_id
		);
...
CREATE TABLE sup.ranking (
	...
	ranking_order_count NUMBER(38), /* 상품 판매 수 */
...
COMMENT ON COLUMN sup.ranking.ranking_order_count IS '상품 판매 수';
...
CREATE TABLE sup.qna (
	qna_id NUMBER(38) NOT NULL, /* Q&A ID */
	sender_id VARCHAR2(20) NOT NULL, /* 보내는사람 */
	recepient_id VARCHAR2(20) NOT NULL, /* 받는사람 */
	product_id NUMBER(38) NOT NULL, /* 상품 아이디 */
	qna_content VARCHAR2(4000) NOT NULL, /* Q&A 내용 */
	prev_qna_id NUMBER(38), /* 이전 Q&A ID */
	qna_checked CHAR(1) NOT NULL, /* 글확인여부 */
	qna_date DATE NOT NULL /* 갱신 날짜 */
);

COMMENT ON TABLE sup.qna IS 'Q&A';

COMMENT ON COLUMN sup.qna.qna_id IS 'Q&A ID';
...
COMMENT ON COLUMN sup.qna.qna_content IS 'Q&A 내용';

COMMENT ON COLUMN sup.qna.prev_qna_id IS '이전 Q&A ID';
...
ALTER TABLE sup.order
	ADD
		CONSTRAINT FK_member_TO_order2
		FOREIGN KEY (
			sender_id
		)
		REFERENCES sup.member (
...
ALTER TABLE sup.like_feed
	ADD
		CONSTRAINT FK_post_TO_like_feed
		FOREIGN KEY (
			post_id
		)
		REFERENCES sup.post (
...
ALTER TABLE sup.like_feed
	ADD
		CONSTRAINT FK_member_TO_like_feed
		FOREIGN KEY (
			member_id
		)
		REFERENCES sup.member (
